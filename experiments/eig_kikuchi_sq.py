...
def MaxwellDG(fes):
    ...
    #a += alpha * k**2 / h * jump_p*jump_q*dx(skeleton=True)
...
mesh = Mesh(geo.GenerateMesh(maxh=0.1,comm=MPI.COMM_WORLD))

V = HCurl(mesh, order=order, dgjumps=True, type1=True, dirichlet=".*")
Q = H1(mesh, order=order, dgjumps=True, dirichlet=".*")
fes = V * Q
